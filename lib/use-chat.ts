      ...
      async (chatRequest: ChatRequest) => {
        ...
        try {
          ...
          await callChatApi({
            ...
            onResponse,
            onUpdate({ message, data, replaceLastMessage, singleData }) {
              mutateStatus('streaming');
              // Get the current messages
              const currentMessages = messagesRef.current;
              
              // If this is a new message (different messageId), append it
              const lastMessage = currentMessages[currentMessages.length - 1];
              const isNewMessage = lastMessage?.id !== message.id;
              ...
              if (data?.length) {
                ...
                if (Array.isArray(singleData) && Array.isArray(singleData[0]) && singleData[0].length > 0) {
                  setMessages([
                    ...messagesRef.current,
                    ...singleData[0] as any
                  ]);
              ...
              if (isNewMessage) {
                ...
                throttledMutate([...currentMessages, message], false);
              } else {
