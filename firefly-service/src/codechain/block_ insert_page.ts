...
export interface BlockInsertPageChainInput<T extends string | object = string>
  extends ChainInputs {
...
export default class BlockInsertPageChain<T extends string | object = string>
  extends BaseChain
  implements BlockInsertPageChainInput<T>
{
  ...
  similarityText = '将{block_name}加入到{block_insert_page_placeHolder}';

  outputParser?: BaseOutputParser<T>;

  constructor(fields: BlockInsertPageChainInput<T>) {
    super(fields);
  ...
  _chainType(): string {
    return 'block_ insert_page_chain' as const;
  }
